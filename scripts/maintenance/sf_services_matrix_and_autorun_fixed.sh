...
# جمع الخدمات بشكل صحيح
services=$(systemctl list-unit-files 'sf-*' 'ff-*' --no-legend | awk '{print $1}' | tr '\n' ' ')
log "تم العثور على الخدمات: $services"
...
log "تم تحليل $service_count خدمة"
...
for layer in "${!layers[@]}"; do
    service_list="${layers[$layer]}"
    if [ -n "$service_list" ]; then
        count=$(echo "$service_list" | wc -w)
...
brain_only() {
    ...
    systemctl restart sf-health.service 2>/dev/null && echo "   ✅ sf-health.service" || echo "   ❌ sf-health.service"
    systemctl restart sf-memory.service 2>/dev/null && echo "   ✅ sf-memory.service" || echo "   ❌ sf-memory.service"
    systemctl restart sf-unified.service 2>/dev/null && echo "   ✅ sf-unified.service" || echo "   ❌ sf-unified.service"
    systemctl restart ff-healthd.service 2>/dev/null && echo "   ✅ ff-healthd.service" || echo "   ❌ ff-healthd.service"
}

suite_core() {
    echo "🏗️ تشغيل السويت الأساسي"
    brain_only
    systemctl restart sf-bot.service 2>/dev/null && echo "   ✅ sf-bot.service" || echo "   ❌ sf-bot.service"
    systemctl restart sf-telegram.service 2>/dev/null && echo "   ✅ sf-telegram.service" || echo "   ❌ sf-telegram.service"
    systemctl restart sf-learning.service 2>/dev/null && echo "   ✅ sf-learning.service" || echo "   ❌ sf-learning.service"
    systemctl restart sf-spider.service 2>/dev/null && echo "   ✅ sf-spider.service" || echo "   ❌ sf-spider.service"
}
...
full_suite() {
    ...
    for service in $(systemctl list-unit-files 'sf-*' 'ff-*' --no-legend | awk '{print $1}'); do
        if systemctl is-enabled "$service" >/dev/null 2>&1; then
            ...
            systemctl restart "$service" 2>/dev/null && echo "      ✅" || echo "      ❌"
        fi
    done
}
...
status_report() {
    ...
    echo "   الخدمات النشطة:"
    systemctl list-units 'sf-*' 'ff-*' --state=running --no-legend | awk '{print "      ✅ " $1}' || echo "      لا توجد خدمات نشطة"
    echo "   الخدمات المعطلة:"
    systemctl list-units 'sf-*' 'ff-*' --state=inactive --no-legend | awk '{print "      💤 " $1}' || echo "      لا توجد خدمات معطلة"
    echo "   الخدمات الفاشلة:"
    systemctl list-units 'sf-*' 'ff-*' --state=failed --no-legend | awk '{print "      💀 " $1}' || echo "      لا توجد خدمات فاشلة"
}
...
# إحصائيات الخدمات
active_count=$(systemctl list-units 'sf-*' 'ff-*' --state=active --no-legend | wc -l)
inactive_count=$(systemctl list-units 'sf-*' 'ff-*' --state=inactive --no-legend | wc -l)
failed_count=$(systemctl list-units 'sf-*' 'ff-*' --state=failed --no-legend | wc -l)
...
📈 الإحصائيات:
• إجمالي الخدمات المحللة: $service_count
• الخدمات النشطة: $active_count
• الخدمات المعطلة: $inactive_count  
• الخدمات الفاشلة: $failed_count
